...
# GIT PROMPT
if [ ! -n "${ZSH_THEME_GIT_PROMPT_PREFIX+1}" ]; then
  ZSH_THEME_GIT_PROMPT_PREFIX=" \ue0a0 "
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_SUFFIX+1}" ]; then
  ZSH_THEME_GIT_PROMPT_SUFFIX=""
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_DIRTY+1}" ]; then
  ZSH_THEME_GIT_PROMPT_DIRTY=" ✘"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_CLEAN+1}" ]; then
  ZSH_THEME_GIT_PROMPT_CLEAN=" ✔"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_ADDED+1}" ]; then
  ZSH_THEME_GIT_PROMPT_ADDED=" %F{green}✚%F{black}"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_MODIFIED+1}" ]; then
  ZSH_THEME_GIT_PROMPT_MODIFIED=" %F{blue}✹%F{black}"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_DELETED+1}" ]; then
  ZSH_THEME_GIT_PROMPT_DELETED=" %F{red}✖%F{black}"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_UNTRACKED+1}" ]; then
  ZSH_THEME_GIT_PROMPT_UNTRACKED=" %F{yellow}✭%F{black}"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_RENAMED+1}" ]; then
  ZSH_THEME_GIT_PROMPT_RENAMED=" ➜"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_UNMERGED+1}" ]; then
  ZSH_THEME_GIT_PROMPT_UNMERGED=" ═"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_AHEAD+1}" ]; then
  ZSH_THEME_GIT_PROMPT_AHEAD=" ⬆"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_BEHIND+1}" ]; then
  ZSH_THEME_GIT_PROMPT_BEHIND=" ⬇"
fi
if [ ! -n "${ZSH_THEME_GIT_PROMPT_DIVERGED+1}" ]; then
